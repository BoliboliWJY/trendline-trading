...
def filter_trend_initial(trend_high, trend_low, data, config):
    ...
    if config.get("filter_reverse", False):
        trend_high = filter_by_reverse(trend_high, True)
        trend_low = filter_by_reverse(trend_low, False)

    # 斜率大小限制
    if config.get("filter_slope", False):
        trend_high = filter_by_slope(
            trend_high, threshold=config.get("slope_threshold", 1)
        )
        trend_low = filter_by_slope(
            trend_low, threshold=config.get("slope_threshold", 1)
        )

    # 最小生成间隔
    if config.get("filter_line_age", False):
        trend_high = filter_by_line_age(
    ...
    if config.get("filter_distance", False):
        ...
        trend_high = filter_by_distance(
            trend_high,
            data,
            distance_threshold=config.get("distance_threshold", 10),
            interval=interval,
        )
        trend_low = filter_by_distance(
            trend_low,
            data,
            distance_threshold=config.get("distance_threshold", 10),
            interval=interval,
...
def filter_by_reverse(trend, is_high):
    """过滤反转趋势"""
...
def filter_trend(
    ...
    if filter_reverse:
        trend_high = filter_by_reverse_last(trend_high, True)
        trend_low = filter_by_reverse_last(trend_low, False)
    # 斜率大小限制
    if filter_slope:
        trend_high = filter_by_slope_last(trend_high, threshold=config.get("slope_threshold", 1))
        trend_low = filter_by_slope_last(trend_low, threshold=config.get("slope_threshold", 1))
    # 趋势年龄限制
    if filter_line_age:
        trend_high = filter_by_line_age_last(
            trend_high, min_age=config.get("min_line_age", 5)
        )
        trend_low = filter_by_line_age_last(
            trend_low, min_age=config.get("min_line_age", 5)
        )
    # 距离限制
    if filter_distance:
        trend_high = filter_by_distance_last(
            trend_high,
            data,
            distance_threshold=config.get("distance_threshold", 10),
            interval=interval,
        )
    trend_low = filter_by_distance_last(
            trend_low,
            data,
            distance_threshold=config.get("distance_threshold", 10),
            interval=interval,
...
def filter_by_reverse_last(trend, is_high):
    """过滤反转趋势"""
    new_last_row = []
