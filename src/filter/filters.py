...
class trend_filter:
    def __init__(self, initial_trend_data, data, config):
    ...
    def filter_trend_initial(self, trend_high, trend_low, data, config):
        """
        初始化过滤
        """
        ...
        # 反转限制
        delay = config.get("delay", 10) - 1
        trend_high = trend_high[:-delay]
        trend_low = trend_low[:-delay]

        if config.get("filter_reverse", False):
        ...
        if config.get("filter_slope", False):
            trend_high = self.filter_by_slope(
                trend_high, threshold=config.get("slope_threshold", 1), is_high=True
            )
            trend_low= self.filter_by_slope(
                trend_low, threshold=config.get("slope_threshold", 1), is_high=False
            )

        # 最小生成间隔
        if config.get("filter_line_age", False):
            min_age_value = config.get("min_line_age", 5)
        ...
        if config.get("filter_distance", False):
            ...
            trend_high = self.filter_by_distance(
                trend_high,
                data,
                distance_threshold=config.get("distance_threshold", 10),
                interval=interval,
                is_high=True
            )
    ...
    def filter_by_reverse(self,trend, is_high):
        ...
        for i in range(1, len(trend)):
            valid_slopes = [slope for slope, j in trend[i] if (is_high and slope < 0) or (not is_high and slope > 0)]
            filtered_trend[i].extend([[slope, j] for slope, j in trend[i] if slope in valid_slopes])
    ...
    def filter_by_trending_line(self, trend, is_high):
        ...
        for i in range(1, len(trend)):
            for slope, j in trend[i]:
                ...
                filtered_trend[i].append([slope, j])
        return filtered_trend
    def filter_trend(self, original_trend_high, original_trend_low, trend_high, trend_low, data, config):
        """
        过滤趋势,只过滤最后一个元素
        Args:
        ...
        if not enable_filter:
            return (
                self.filter_by_trending_line_last(trend_high, original_trend_high, 0),
                self.filter_by_trending_line_last(trend_low, original_trend_low, 0),
            )
        ...
        # 斜率大小限制
        if filter_slope:
            trend_high = self.filter_by_slope_last(trend_high, threshold=config.get("slope_threshold", 1))
            trend_low = self.filter_by_slope_last(trend_low, threshold=config.get("slope_threshold", 1))
        # 趋势年龄限制
        if filter_line_age:
            trend_high = self.filter_by_line_age_last(
                trend_high, min_age=config.get("min_line_age", 5)
            )
            trend_low = self.filter_by_line_age_last(
                trend_low, min_age=config.get("min_line_age", 5)
            )
        # 距离限制
        if filter_distance:
