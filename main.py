        ...
        if visualize_mode:
            ...
            with tqdm(desc="Backtesting Progress", mininterval=1) as pbar:
                for current_trend in backtester.run_backtest():
                    ...
                    trend_tick_data, latest_trend_tick_data = trend_tick_calculator.update_trend_data(data, filtered_trend_data, data[base_trend_number, 0], data[base_trend_number, 2])
                    

                    trader.update_trend_price(data, trend_tick_data, latest_trend_tick_data, base_trend_number, len(filtered_trend_data["deleted_high"]), len(filtered_trend_data["deleted_low"]), filtered_trend_data["trend_high"][-1], filtered_trend_data["trend_low"][-1])
